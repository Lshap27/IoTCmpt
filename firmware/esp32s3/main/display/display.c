...
static void tft_flush(void) {
    uint8_t data[4];
    data[0] = 0x00; data[1] = 0x02; data[2] = 0x00; data[3] = 0x81;
    tft_cmd(ST7735_CASET);
    tft_data(data, 4);

    data[0] = 0x00; data[1] = 0x01; data[2] = 0x00; data[3] = 0x80;
...
esp_err_t display_render(const sensor_sample_t *sample, const fusion_state_t *state,
                          const app_status_t *status) {
    if (!CONFIG_APP_DISPLAY_ENABLED) {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (sample->climate_valid) {
        uint16_t tc = (sample->temperature_c > 32.0f)   ? COLOR_RED
                      : (sample->temperature_c > 28.0f) ? COLOR_YELLOW
                                                        : COLOR_GREEN;
        uint16_t hc = (sample->humidity_percent > 75.0f || sample->humidity_percent < 30.0f)
                          ? COLOR_YELLOW
                          : COLOR_GREEN;
        int tx = 4;
    ...
    /* ── TVOC / eCO2 行 (y=34) ── */
    if (sample->air_valid) {
        uint16_t vc = (sample->tvoc_ppb > 600)   ? COLOR_RED
                      : (sample->tvoc_ppb > 300) ? COLOR_YELLOW
                                                 : COLOR_GREEN;
        uint16_t cc = (sample->eco2_ppm > 1500)   ? COLOR_RED
                      : (sample->eco2_ppm > 1000) ? COLOR_YELLOW
                                                  : COLOR_GREEN;
        int tx = 4;
    ...
    /* ── HCHO / 光照行 (y=50) ── */
    {
        ...
        if (sample->air_valid) {
            ...
            fc = (sample->hcho_ug_m3 > 100)   ? COLOR_RED
                 : (sample->hcho_ug_m3 > 60)  ? COLOR_YELLOW
                                              : COLOR_GREEN;
        }
        int tx = 4;
    ...
    /* ── 窗户状态面板 (y=104, h=18) ── */
    {
        ...
        if (control.manual_override) {
            fb_draw_string_center(105,
                                  control.manual_open ? "[MANUAL] OPEN" : "[MANUAL] CLOSED",
                                  COLOR_YELLOW, COLOR_PANEL_BG);
        } else {
            fb_draw_string_center(105,
                                  control.window_open ? "[AUTO] OPEN" : "[AUTO] CLOSED",
                                  control.window_open ? COLOR_GREEN : COLOR_GRAY, COLOR_PANEL_BG);
        }
    }

    /* ── MQTT 状态 (y=123) ── */
    fb_draw_string_center(123,
                          status->cloud == APP_STATUS_LINK_READY ? "MQTT: ONLINE" : "MQTT: --",
                          status->cloud == APP_STATUS_LINK_READY ? COLOR_BLUE : COLOR_GRAY,
                          COLOR_DARK_BG);
