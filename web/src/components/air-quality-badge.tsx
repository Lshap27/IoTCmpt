...
export function AirQualityBadge({ level, className }: { level: string | null | undefined; className?: string }) {
  const entry = LEVELS[level ?? "unknown"] ?? LEVELS.unknown;
  const { Icon } = entry;
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border border-line px-2.5 py-1 text-xs font-medium text-ink2 ${className ?? ""}`}
