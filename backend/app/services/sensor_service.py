...
def create_sensor_reading(db: Session, data: SensorUpload) -> SensorReading:
    reading = SensorReading(
        ...
        dehumidifier_state=data.dehumidifier_state,
...
def reading_payload(reading: Optional[SensorReading], pose: Optional[PoseEvent]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "led_status": reading.led_status if reading else None,
        "window_status": reading.window_status if reading else None,
        "dehumidifier_state": reading.dehumidifier_state if reading else None,
